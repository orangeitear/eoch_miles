...
/**
 * 位置误差补偿：通过PD控制器将位置误差转换为期望加速度
 * 姿态解算：给鸡加速度向量转化为无人机的期望姿态角
 * 推力计算：根据加速Z分量和推力模型计算总推力
 * 模型自适应：使用传感器反馈在线更新推理模型参数
 */
using namespace std;



double LinearControl::fromQuaternion2yaw(Eigen::Quaterniond q) //姿态转换函数，将四元数转换为偏航角
{
    ...
    Controller_Output_t &u)
{
  /* WRITE YOUR CODE HERE */
      ...
      //计算加速度(PID控制) 加速度 = 期望加速度 + 速度环增益*(期望速度-实际速度) + 位置环增益*(期望位置-实际位置) + 重力补偿(0,0,g)
      des_acc = des.a + Kv.asDiagonal() * (des.v - odom.v) + Kp.asDiagonal() * (des.p - odom.p);
      des_acc += Eigen::Vector3d(0,0,param_.gra);
      //计算总推力,这里将期望加速度的z轴分量转化为油门百分比
